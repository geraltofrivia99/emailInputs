fetch("https://video.google.com/timedtext?lang=en&v=5MgBikgcWnY").then(res => res.text()).then(str => new DOMParser().parseFromString(str,"text/xml")).then(doc => {
  console.log(doc);
  const obj = [].slice.call(doc.getElementsByTagName('text')).map(text => ({
